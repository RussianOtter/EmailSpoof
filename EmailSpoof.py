import re, smtplib, argparse, logging, sqlite3, uuid, time, random, mimetypes
...
def is_domain_spoofable(from_address, to_address):
	...
	output_info("Checking if from domain is spoofable")
	...
	try:
		...
		for to_address in to_addresses:
			...
			if args.slow_send:
				delay_send()
				output_info("Connecting to SMTP server at " + args.smtp_server + ":" + str(args.smtp_port))
				server = smtplib.SMTP(args.smtp_server, args.smtp_port, None, 10)
